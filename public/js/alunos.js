...
async function fetchRecados() {
    ...
    // Aplica o grid responsivo para ficar um do lado do outro (até 5 colunas se a tela for muito larga). min() garante responsividade em telas muito estreitas.
    container.style.cssText = 'display: grid; grid-template-columns: repeat(auto-fill, minmax(min(100%, 280px), 1fr)); gap: 1.5rem;';
...
function initCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        windowResize: function (arg) {
            ...
            } else if (!mobile && calendar.view.type !== 'dayGridWeek') {
                calendar.changeView('dayGridWeek');
            }
        },

        // Customização do cabeçalho da data para exibir o nome do feriado embaixo do dia (somente dayGrid)
        dayHeaderContent: function (arg) {
            const diaStr = arg.text.toUpperCase(); // ex: SEG. 27/04

            if (arg.view.type === 'listWeek') {
                return { html: `<div style="font-size: 1.1rem; font-weight: 600; color: #3b82f6; text-transform: capitalize;">${arg.text}</div>` };
            }
        ...
        // Customização visual: Adapta o conteúdo do evento com base na view
        eventContent: function (arg) {
            // Se for lista
            if (arg.view.type === 'listWeek') {
                if (arg.event.extendedProps.isHoliday) {
                    return {
                        html: `<div style="color: #ef4444; font-weight: bold; text-transform: uppercase;">FERIADO: ${arg.event.title}</div>`
                    };
                }
                ...
                return {
                    html: `
                    <div style="font-size: 1.15rem; font-weight: 700; color: #1e293b; padding: 6px 0;">
                        ${hora}
                    </div>
                    `
                };
            ...
            // Feriados no dayGrid usam o tipo background
            ...
            // Feriados inseridos como evento normal no listWeek não devem aparecer como bolinhas normais no dayGrid
            if (arg.event.extendedProps.isHoliday && arg.view.type === 'dayGridWeek') {
                return { html: '' }; // O background já cuida disso
            }
...
async function fetchAllEvents(fetchInfo, successCallback, failureCallback) {
    ...
    try {
        ...
        // Cria eventos de feriado
        const holidayEvents = [];
        holidaysData.forEach(h => {
            // 1. Feriado de Fundo para o dayGridWeek
            holidayEvents.push({
                start: h.date,
                display: 'background',
                color: '#fee2e2'
            ...
            // 2. Feriado como evento listado para o listWeek
            holidayEvents.push({
                start: h.date,
                title: h.name,
                allDay: true, // Para aparecer no topo do dia
                color: '#ef4444', // Vermelho do feriado
